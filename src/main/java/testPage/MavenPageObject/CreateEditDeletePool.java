...
public class CreateEditDeletePool extends BasePge {
	...
	public void TC03_CreateEditDeletePool() throws InterruptedException{
		...
		asrt.assertEquals(poolName, createEditDeletePoolTestdata.TC03_CreateEditDeletePool, "User is able to enter the pool name");
		...
		asrt.assertTrue(base.isExists(loginObj.Btn_SignInButton("OK")), "User get an alert 'The pool name is valid'");
	}
	//<Summary>
	...
	public void TC04_CreateEditDeletePool() throws InterruptedException{
		...
		base.buttonClick(createEditDeletePoolObj.Btn_CreatePoolAlert("submit", "CREATE"));
		asrt.assertTrue(base.isExists(loginObj.Edt_AlertText("Pool Name is required!..")), "User will not receive an alert message as 'The Pool name field is required'");
	}
	//<Summary>
		...
		public void TC06_CreateEditDeletePool() throws InterruptedException{
			...
			Thread.sleep(4000);
			base.selectorByVisibleText(loginObj.Edt_LoginEmail("pool_userSearch"), poolType);
			/*//base.autoSuggestiveDropDown(createEditDeletePoolObj.Ddl_addUser("ui-id-84"),createEditDeletePoolTestdata.TC06_CreateEditDeletePool_addUser);
           // String poolUsers=base.GetValue(loginObj.Edt_LoginEmail("pool_userSearch"));
           // asrt.assertEquals(poolUsers, createEditDeletePoolTestdata.TC06_CreateEditDeletePool_addUser);
            
			//step 8:Click Create
			//Expected:The user can add multiple users to that pool. So that users can also have the access to this pool.
			base.buttonClick(createEditDeletePoolObj.Btn_CreatePoolAlert("submit", "CREATE"));
			asrt.assertTrue(base.isExists(loginObj.Btn_SignInButton("OK")), "User cannot add multiple users to the pool.");*/
		}
}
